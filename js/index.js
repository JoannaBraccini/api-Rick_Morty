...
async function showFooter() {
  ...
  footerInfo.innerHTML = `
    <div class="col d-flex justify-content-center gap-sm-5 gap-3 align-items-center small text-sm-fs-6 py-4">
      <p>PERSONAGENS: <span class="text-light char-info">${totalCharacters}</span></p>
      <p>LOCALIZAÇÕES: <span class="text-light char-info">${totalLocations}</span></p>
      <p>EPISÓDIOS: <span class="text-light char-info">${totalEpisodes}</span></p>
    </div>  
  `;
}
...
function showCards(cards) {
  ...
  if (limit >= 6) {
    //se tela comum
    for (let i = 0; i < 5; i++) {
      // Inserir os primeiros cinco cards
      if (i < cards.length) {
        const card = cards[i];
        const newCard = createCard(card);
        listCards.appendChild(newCard);
    ...
    // Inserir o sexto card se houver
    if (cards.length > 5) {
      const card = cards[5];
      const newCard = createCard(card);
      listCards.appendChild(newCard);
...
function createCard(card) {
  ...
  newCard.classList.add("col-12", "col-md-5", "mb-4");
  newCard.innerHTML = `
      <div class="card">
          <div class="row g-0">
              <div class="col-4">
                  <img src="${
                    card.image
                  }" class="img rounded-start float-start" alt="${card.name}">
              </div>
              <div class="col-8">
                  <div class="card-body ps-4 py-1 fw-bold text-light">
                      <p class="card-title fs-5 lh-1 lh-md-base" id="name">${
                        card.name
                      }</p>
                      <p class="small card-text lh-1" id="status-species">
                        <span class="status-dot ${
                          card.status === "Alive" ? "bg-success" : "bg-danger"
                        }"></span>                        
                        ${card.status} - ${card.species}</p>
                      <p class="small card-text text-secondary lh-1">Última localização conhecida</p>
                      <p class="small card-text lh-1" id="location">${
                        card.location.name
                      }</p>
                      <p class="small card-text text-secondary lh-1">Visto a última vez em:</p>
                      <p class="small card-text lh-1" id="episode">${
                        card.lastEpisode.name
                      }</p>
                  </div>
              </div>
          </div>
...
function createPagination() {
  ...
  if (endPage - startPage < maxPages - 1) {
    //ajustar a exibição de paginação
