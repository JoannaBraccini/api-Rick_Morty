...
function showCards(cards) {  
    ...
    if(limit >= 6) {    //se tela comum
      for (let i = 0; i < 5; i++) { // Inserir os primeiros cinco cards
          if (i < cards.length) {
              const card = cards[i]
              const newCard = createCard(card)
              listCards.appendChild(newCard)
...
function createCard(card) {
  ...
  newCard.innerHTML = `
      <div class="card">
          <div class="row g-0">
              <div class="col-4">
                  <img src="${card.image}" class="img rounded-start float-start" alt="${card.name}">
              </div>
              <div class="col-8">
                  <div class="card-body ps-4 py-1 fw-bold text-light">
                      <p class="card-title fs-5 lh-1 lh-md-base" id="name">${card.name}</p>
                      ...
                      <p class="small card-text lh-1" id="location">${card.location.name}</p>
                      <p class="small card-text text-secondary lh-1">Visto a última vez em:</p>
                      <p class="small card-text lh-1" id="episode">${card.lastEpisode.name}</p>
                  </div>
              </div>
          </div>
      </div>
